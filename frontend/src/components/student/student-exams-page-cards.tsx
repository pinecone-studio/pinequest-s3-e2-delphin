  ...
  return (
    <article className={cardClassName}>
      <ExamCardTop
        ...
        badge={<span className="inline-flex h-[22px] items-center gap-1.5 rounded-full bg-[#FFF3E0] px-[12px] py-1 text-[12px] font-semibold leading-[1.2] text-[#FF9500] dark:bg-[#FF9500] dark:text-[#FFF3E0] sm:h-[28px] sm:px-[14px] sm:text-[14px]"><span className="h-[8px] w-[8px] rounded-full bg-current dark:bg-[#FFF3E0]" />Удахгүй</span>}
        subtitle={<div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[14px] font-medium leading-[17px] text-[#566069] dark:text-[#E1E6EB] sm:text-[16px] sm:leading-[19px]"><span className="inline-flex items-center gap-1 whitespace-nowrap"><Clock3 className="h-[16px] w-[16px]" />{exam.duration} мин</span><span className="hidden text-[#566069] dark:text-[#E1E6EB] sm:inline">|</span><span className="text-[#566069] dark:text-[#E1E6EB]">{formatScheduleLabel(schedule?.date, schedule?.time)}</span><span className="hidden text-[#566069] dark:text-[#E1E6EB] sm:inline">|</span><span className="whitespace-nowrap">{exam.questions.length} асуулт</span></div>}
