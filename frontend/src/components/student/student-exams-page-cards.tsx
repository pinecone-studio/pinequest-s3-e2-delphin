  ...
  return (
    <article className={cardClassName}>
      <ExamCardTop
        ...
        badge={<span className="inline-flex h-[22px] w-[79px] items-center justify-center gap-1.5 rounded-full bg-[#FFF3E0] px-[9px] py-1 text-[12px] font-semibold leading-[1.2] text-[#FF9500] dark:bg-[#FF9500] dark:text-[#FFF3E0]"><span className="h-[6px] w-[6px] rounded-full bg-current dark:bg-[#FFF3E0]" />Удахгүй</span>}
        subtitle={<div className="grid grid-cols-[auto_1fr_auto] items-center gap-3 text-[14px] font-medium leading-[17px] text-[#566069] dark:text-[#E1E6EB] sm:flex sm:flex-wrap sm:items-center sm:gap-x-4 sm:gap-y-2"><span className="inline-flex items-center gap-1 whitespace-nowrap"><Clock3 className="h-[14px] w-[14px]" />{exam.duration} мин</span><span className="text-center whitespace-nowrap">{formatScheduleLabel(schedule?.date, schedule?.time)}</span><span className="justify-self-end whitespace-nowrap">{exam.questions.length} асуулт</span></div>}
        action={<Link href={href}><Button variant="outline" className={`${actionButtonClassName} border-[#E6F2FF] bg-[#E6F2FF] text-[#007FFF] shadow-[0px_9px_4px_rgba(201,201,201,0.01),0px_5px_3px_rgba(201,201,201,0.05),0px_2px_2px_rgba(201,201,201,0.09),0px_1px_1px_rgba(201,201,201,0.10)] hover:bg-[#DDECFF] ${darkPrimaryActionButtonClassName}`}>Дэлгэрэнгүй</Button></Link>}
      />
    </article>
  );
}
